...
## People don't think the way you think

Emails that need a decoder ring. Slack messages with no clear ask. A draft document so misframed you'd rather start over yourself.

No matter how many times you explain it, the pattern repeats.

Every person who works with you is running a private model of how you think: what you care about, how you decide under pressure, how you like to receive information, what earns your trust and what loses it, what you actually mean when you say "good enough." Nobody handed them that model. They assembled it by watching you in meetings, getting feedback they didn't expect, and adjusting.

The people closest to you have a decent version. Everyone else has a sketch. Every new hire starts from zero.
...
One document, two installs, and the return is different in each.
...
A manual that describes who you'd like to be instead of how you actually operate is worse than no manual, and the reason is new.

Your team corrects for it. Quietly, over time, but they do. Someone reads "bring me the recommendation, not the problem," watches you interrupt three recommendations to ask for the raw data, and updates their private model of you without ever mentioning it.

**An AI has no private model to update.** It reads what you wrote, takes it as fact, and operates as that leader in every output it produces. Write that you want brevity when you actually want the reasoning shown and you'll get clipped summaries forever, each one faithfully executing a document about someone who doesn't exist.

So every section of the template carries the same question. When did this last happen? A principle with no instance behind it is a preference wearing better clothes, and it's the one thing a machine can't catch for you.
...
| Path | What it is |
|---|---|
| [`template.md`](template.md) | The blank spine. Eight sections, each with the evidence question. Start here |
| [`tools/paste-prompt.md`](tools/paste-prompt.md) | Paste into Claude or ChatGPT and get a V1 back, plus the questions that sharpen it |
| [`field-guide.md`](field-guide.md) | The method. The two layers, what a real section looks like, the four failure modes, how to hand it to your team |
| [`example-manual.md`](example-manual.md) | Michael's own manual, unedited. Proof, not a template |
| [`install-in-your-ai.md`](install-in-your-ai.md) | The four surfaces, and a test that can actually fail |
| [`tools/claude-skill/SKILL.md`](tools/claude-skill/SKILL.md) | Install as a Claude skill and it builds the manual with you |
| [`tools/chatgpt/setup.md`](tools/chatgpt/setup.md) | Build kit for the ChatGPT version |

## Start here

1. Open [`template.md`](template.md) and read the eight sections. Two minutes.
2. Paste [`tools/paste-prompt.md`](tools/paste-prompt.md) into whichever AI already knows something about how you work. You'll get a V1 and five questions. Thirty minutes.
3. Run the recognition test. One person who works closely with you, one question: does this match your experience of me? Don't defend it while they answer.
4. Install it. [`install-in-your-ai.md`](install-in-your-ai.md), four surfaces, five minutes.
5. Then send it to your team, and ask each of your leaders to write theirs.

Read [`example-manual.md`](example-manual.md) somewhere in there, for depth and for how specific the specifics get. Not to fill in.

## Don't copy the example

Michael gave his manual to three peers in an advisory group. Inside 48 hours, all three had forwarded it to their own teams with most of his language still in it.

The document worked. It just wasn't theirs.

Use his for structure and depth. The exercise is the articulation, and that's where the whole value sits. A borrowed manual installs somebody else's operating system in your company, which is a strange thing to do on purpose.
...
[CC BY 4.0](LICENSE). Use it, adapt it, reword it until it sounds like you, install it wherever you keep your standards. Attribution is required by the license and takes one line.
